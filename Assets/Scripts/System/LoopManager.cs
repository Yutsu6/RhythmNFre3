...
public class LoopManager : MonoBehaviour
{
    ...
    public bool ShouldSkipRow(int rowId)
    {
        foreach (var loop in activeLoops)
        {
            if (IsRowInLoopBody(rowId, loop) && loop.remainingLoops == 0)
            {
                Debug.Log($"跳过行{rowId}: 在循环{loop.startRowId}体内但循环次数为0");
                return true;
            }
        }
        return false;
    ...
    {
        // 找到当前行所在的最内层循环
        var innermostLoop = FindInnermostLoopForRow(currentRowId);
        if (innermostLoop != null && innermostLoop.remainingLoops == 0)
        {
            // 跳到该循环体外的下一行
            var sortedRowIds = parser.GetSortedRowIds();
            int endIndex = sortedRowIds.IndexOf(innermostLoop.endRowId);

            if (endIndex + 1 < sortedRowIds.Count)
            {
                int skipTarget = sortedRowIds[endIndex + 1];
                Debug.Log($"跳过循环体{innermostLoop.startRowId}: {currentRowId} -> {skipTarget}");
                return skipTarget;
            }
        }
